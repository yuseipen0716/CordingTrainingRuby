...
# reviewしていただき、以下のように修正しました。
# 1から10までのグループそれぞれの合計を足し合わせる繰り返し処理①
(1..10).each do |i|
    # switch == 1 の場合のbreakの記述はワンラインに。
    break if switch == 1
    # 以下はswitch == 0の場合の処理(各グループの合計を出すための繰り返し処理②)
    puts "■第#{i}グループ"
    # ここで各グループの合計を初期化。totalを定義したところに書いてしまうと、計算したsumが加算されたままtotalに足されるため答えが合わなくなる。/
    # /（1グループの合計＋(1グループの合計 + 2グループの合計) + (1,2,3グループの合計)・・・という風になってしまう！
    sum = 0
    (1..5).to_a.each do |j|
        ...
        if num == 88888
            ...
            break
        # 99999が入力された際には、合計から99999を引いて、switchを1にしてから繰り返し処理②を離脱。繰り返し処理①へ。
        # 88888のときと、99999の時以外はこれらの条件をパスして繰り返し処理②を継続できるように、ここはelseではなく、elsifで記載。
        elsif num == 99999
            sum -= num 
            switch = 1
            break
